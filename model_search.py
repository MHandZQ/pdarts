...
class MixedOp(nn.Module):
    ...
    def __init__(self, C, stride, switch, p):
        ...
        for i in range(len(switch)):
            # 这里switch长为8,里面存储的是True或者False,从而实现对operation的选择
            # 如果为True,则对应索引的操作被选择,如果为False则丢弃,实现训练过程中搜索空间的减小
            if switch[i]:
                primitive = PRIMITIVES[i]
                op = OPS[primitive](C, stride, False)
                if 'pool' in primitive:
                    op = nn.Sequential(op, nn.BatchNorm2d(C, affine=False))
                # 如果操作时skip-connect,则使用Dropout Rate
                # 所以原文说的操作级别的Dropout，就是指对skip-connect
    ...
    def __init__(self, steps, multiplier, C_prev_prev, C_prev, C, reduction, reduction_prev, switches, p):
        ...
        for i in range(self._steps):
            for j in range(2+i):
                ...
                # switch：通过switches控制选择哪一个操作,即实现搜索过程中搜索空间的减小
                # p：即Dropout Rate
