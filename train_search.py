...
def main():
    ...
    if args.cifar100:
        train_data = dset.CIFAR100(root=args.data_dir, train=True, download=True, transform=train_transform)
    else:
    ...
    # switch包含14个长为8的list,每个list中存储的都是True
    # switch:[[True, True, True, True, True, True, True, True], [True, True, True, True, True, True, True, True], [True, True, True, True, True, True, True, True], [True, True, True, True, True, True, True, True], [True, True, True, True, True, True, True, True], [True, True, True, True, True, True, True, True], [True, True, True, True, True, True, True, True], [True, True, True, True, True, True, True, True], [True, True, True, True, True, True, True, True], [True, True, True, True, True, True, True, True], [True, True, True, True, True, True, True, True], [True, True, True, True, True, True, True, True], [True, True, True, True, True, True, True, True], [True, True, True, True, True, True, True, True]]
    switches = []
    for i in range(14):
    ...
    # To be moved to args
    # 每一个Stage结束后保留的搜索空间的大小,即每一个边保留的操作的数目
    num_to_keep = [5, 3, 1]
    # 每一个Stage结束后丢弃的操作的数目,初始是8丢弃3个得到5,再丢弃两个得到2,最后再丢弃两个得到1
    num_to_drop = [3, 2, 2]
    # 改变网络初始通道数
    ...
    # 初始阶段为5层,然后在中间阶段增加了6层得到11层,然后在最后阶段加了12层得到17层
        add_layers = [0, 6, 12]
    # 设置每个阶段的初始dropout rate
    if len(args.dropout_rate) ==3:
        drop_rate = args.dropout_rate
    else:
        drop_rate = [0.0, 0.0, 0.0]

    # 只训练网络参数 w 的epochs,原文中是10
    eps_no_archs = [10, 10, 10]
    # sp(search phase):搜索阶段,3个
    for sp in range(len(num_to_keep)):
        ...
        for epoch in range(epochs):
            ...
            # training
            # 前10个epochs只更新网络的权重参数
            if epoch < eps_no_arch:
                # 每一个epoch改变一次dropout rate
                model.module.p = float(drop_rate[sp]) * (epochs - epoch - 1) / epochs
                model.module.update_p()
                train_acc, train_obj = train(train_queue, valid_queue, model, network_params, criterion, optimizer, lr, architect, train_arch=False)
            # 后15个epochs既更新网络的权重参数又更新网络的架构参数
            else:
                # 每一个epoch改变一次dropout rate
                model.module.p = float(drop_rate[sp]) * np.exp(-(epoch - eps_no_arch) * scale_factor) 
                model.module.update_p()                
                train_acc, train_obj = train(train_queue, valid_queue, model, network_params, criterion, optimizer, lr, architect, train_arch=True)
        ...
        # 本应保存了4个weights.pt,依次对应操作数为8、5、3、1
        # 但是名字一样会覆盖,到最后保存的就是只剩一个操作的架构权重
        ...
        if sp == len(num_to_keep) - 1:
            ...
            # 这里是记录了no restrict skipconnetct genotype and genotypes with number of skip-connect:7,6,5,4,3,2,1,0
            # 文章中推荐限制数目为2
...
def train(train_queue, valid_queue, model, network_params, criterion, optimizer, lr, architect, train_arch=True):
    objs = utils.AvgrageMeter()
    top1 = utils.AvgrageMeter()
    top5 = utils.AvgrageMeter()

    tbar = tqdm(train_queue, ncols=130)
    for step, (input, target) in enumerate(tbar):
        ...
        if train_arch:
            ...
            # unrolled为False就是本文使用的一阶近似
            # 但我想试一下二阶近似,所以直接用了DARTS中的architect.py
            input_search = input_search.cuda()
            target_search = target_search.cuda(non_blocking=True)
            architect.step(input, target, input_search, target_search, lr, unrolled=args.unrolled)

        # 网络参数更新
        optimizer.zero_grad()

        # TODO：之后把SmoothDARTS中的扰动添加进来看看效果怎么样
...
def infer(valid_queue, model, criterion):
    ...
    tbar = tqdm(valid_queue, ncols=130)
    for step, (input, target) in enumerate(tbar):
        input = input.cuda()
        target = target.cuda(non_blocking=True)
...
# 删除最小权重的跳跃连接
# 如果输入的列表里没有跳跃连接则返回-1；否则返回原列表switches_bk中的跳跃连接索引。
